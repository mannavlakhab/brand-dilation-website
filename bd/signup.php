...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Handle file upload
$profile_icon = "";
if (isset($_FILES['profile_icon']) && $_FILES['profile_icon']['error'] == 0) {
    $target_dir = "../uploads/profiles/"; // Ensure this directory exists and is writable
    $file_name = basename($_FILES['profile_icon']['name']);
    ...
    if ($_FILES['profile_icon']['size'] > 2 * 1024 * 1024) {
    ...
    if (move_uploaded_file($_FILES['profile_icon']['tmp_name'], $target_file)) {
        $profile_icon = $target_file;
    } else {
        echo "Error uploading profile icon.";
        echo "Error Code: " . $_FILES['profile_icon']['error']; // Output the error code
        exit();
    }
} else {
    echo "Error uploading file. Error Code: " . $_FILES['profile_icon']['error']; // Output the error code
    exit();
}
    // Hash the password
    ...
    $stmt = $conn->prepare("INSERT INTO admin_users (profile_icon, firstname, lastname, username, password, role, email, phone, gender, date_of_birth, date_of_joining) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("sssssssssss", $profile_icon, $firstname, $lastname, $username, $hashed_password, $role, $email, $phone, $gender, $date_of_birth, $date_of_joining);
    ...
    echo "File Name: " . $_FILES['profile_icon']['name'] . "<br>";
echo "File Size: " . $_FILES['profile_icon']['size'] . "<br>";
echo "File Type: " . $_FILES['profile_icon']['type'] . "<br>";
echo "Temp Name: " . $_FILES['profile_icon']['tmp_name'] . "<br>";
...
<!-- 
    <div class="bg-blue-600 hidden lg:block w-full md:w-1/3 xl:w-2/3 h-screen">
      <img src="../assets/img/login bg.png" alt="" class="w-full h-full object-cover">
    </div> -->
  
    <div class="mt-10 bg-white w-full md:max-w-md lg:max-w-full md:mx-auto md:mx-0 md:w-1/2 xl:w-1/3  px-6 lg:px-16 xl:px-12
          flex items-center justify-center">
  
      <div class="w-full h-100">
        <img src="../assets/img/64-bd-t.png" alt="LOGO" class="w-16 mx-auto">
        <h1 class="text-xl md:text-2xl font-bold leading-tight mt-12">Register in to BRAND DILATION</h1>
  
        <form class="mt-6" action="" method="POST" enctype="multipart/form-data">
          <div>
            <label class="block text-gray-700">Profile Photo</label>
            <input type="file" name="profile_icon" id="profile_icon" required class="w-full px-4 py-3 rounded-lg bg-gray-200 mt-2 border focus:border-blue-500 focus:bg-white focus:outline-none" autofocus autocomplete required>
          </div>

          <div class="mt-4">
            <label class="block text-gray-700">First Name</label>
            <input type="text" id="firstname" name="firstname" required placeholder="Enter first name" class="w-full px-4 py-3 rounded-lg bg-gray-200 mt-2 border focus:border-blue-500 focus:bg-white focus:outline-none" autofocus autocomplete required>
          </div>

          <div class="mt-4">
            <label class="block text-gray-700">Last name</label>
            <input type="text" id="lastname" name="lastname" required placeholder="Enter last name" class="w-full px-4 py-3 rounded-lg bg-gray-200 mt-2 border focus:border-blue-500 focus:bg-white focus:outline-none" autofocus autocomplete required>
          </div>
          ...
          <div class="mt-4">
            ...
            <select id="role" name="role" class="w-full px-4 py-3 rounded-lg bg-gray-200 mt-2 border focus:border-blue-500
                  focus:bg-white focus:outline-none" required>
                  <option  selected disabled value="">select your role</option>
            <option value="sales">Sales</option>
            <option value="inventory">Inventory</option>
            <option value="outer">Delivery Inventory</option>
            <option value="delivery">Delivery Man/Woman</option>
          ...
          <div  class="mt-4">
            ...
            <input type="email" id="email" name="email" placeholder="Enter email" class="w-full px-4 py-3 rounded-lg bg-gray-200 mt-2 border focus:border-blue-500
                  focus:bg-white focus:outline-none" required>
          </div>

          <div class="mt-4">
            <label class="block text-gray-700">Phone Number</label>
            <input id="phone" name="phone" placeholder="Enter Password"  type="tel" class="w-full px-4 py-3 rounded-lg bg-gray-200 mt-2 border focus:border-blue-500
                  focus:bg-white focus:outline-none" required>
          </div>

          <div class="mt-4">
            <label class="block text-gray-700">Select gender</label>
            <select id="gender" name="gender" placeholder="Enter gender" class="w-full px-4 py-3 rounded-lg bg-gray-200 mt-2 border focus:border-blue-500
                  focus:bg-white focus:outline-none" required>
          ...
          <div class="mt-4">
            <label class="block text-gray-700">Select Your Birth Date</label>
            <input placeholder="Date Of Birth" type="date" name="date_of_birth" id="date_of_birth" class="w-full px-4 py-3 rounded-lg bg-gray-200 mt-2 border focus:border-blue-500
                  focus:bg-white focus:outline-none" required>
          </div>
            
          <div class="mt-4">
            <label class="block text-gray-700">Select Your joining Date</label>
            <input placeholder="Date Of joining" type="date" name="date_of_joining" id="date_of_joining" class="w-full px-4 py-3 rounded-lg bg-gray-200 mt-2 border focus:border-blue-500
                  focus:bg-white focus:outline-none" required>
          </div>
          <button type="submit" class="w-full block bg-blue-500 hover:bg-blue-400 focus:bg-blue-400 text-white font-semibold rounded-lg
                px-4 py-3 mt-6">Done on my side</button>
        </form>
        ...
        <p class="mt-8">
            ...
            <br>
            <button type="button" onclick="window.location.href='./login.php'"  class="w-full block bg-white hover:bg-gray-100 focus:bg-gray-100 text-gray-900 font-semibold rounded-lg px-4 py-3 border border-gray-300">
