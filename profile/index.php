...
while ($order = $order_result->fetch_assoc()) {
    ...
    if ($order_status == 'delivered') {
        ...
        echo "<button onclick=\"location.href = '../refund_tracking.php?ref_track=" . htmlspecialchars($order['ref_track']) . "&user_id=" . $user_id . "';\" class='btn'>Track Refund</button>";
    } elseif (in_array($order_status, ['pickup and delivering', 'exchanage under process'])) {
...
if (isset($_GET['delete'])) {
    ...
    if ($stmt->execute()) {
        header("Location: ../profile?page=address");
        exit();
    } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_address'])) {
    ...
    $stmt->close();
        ...
        if ($stmt = $conn->prepare($addresses_query)) {
            ...
            echo "<br><button class='btn-trick-new' onclick=\"window.location.href='../profile?page=address&editbyman=1'\">Add a new address</button><br><br>";
            echo "<table>";
...
if ($tri_man == 1) {
    ...
    echo "<br><button class='btn-trick-new' onclick=\"window.location.href='../profile?page=address'\">&times; Close Form</button><br>";
    ...
    elseif (isset($_GET['page']) && $_GET['page'] == 'settings') {
        ...
        <br>
         ...
         <a href='?page=address'><button class='btn-trick-new su' onclick='location.href = '';'>Add/Edit Addess</button></a>
